...
**Author:** Nathan Marz (*Thoughts from the Red Planet*) — widely attributed; not stated in the source paste
**Published:** Thursday, October 13, 2011
**Source:** http://nathanmarz.com/blog/how-to-beat-the-cap-theorem.html

> Saved article (verbatim capture, web UI chrome removed; inline architecture diagrams are referenced in prose but not reproduced). This is the foundational **Lambda Architecture** essay — pairs directly with Tyler Akidau's *Streaming 101*, which critiques this exact proposal by name (Kappa vs. Lambda). See [[streaming-101-world-beyond-batch]].

---

The CAP theorem states a database cannot guarantee consistency, availability, and partition-tolerance at the same time. But you can't sacrifice partition-tolerance, so you must make a tradeoff between availability and consistency. Managing this tradeoff is a central focus of the NoSQL movement.
...
- The system uses incremental algorithms to lower the latency of queries to an acceptable level
...
Since a query is a function of all the data, the easiest way to make queries run fast is to precompute them. Whenever there's new data, you just recompute everything. This is feasible because we relaxed the problem to allow queries to be out of date by a few hours.

To build this, you need a system that:

- Can easily store a large and constantly growing dataset
- Can compute functions on that dataset in a scalable way
...
Let's see an example of the realtime layer by going back to the running example of querying the number of page views for a URL over a time range. The batch system is the same as before: a batch workflow based on Hadoop and ElephantDB precomputes the query for everything but the last few hours of data. All that's left is to build the realtime system that compensates for those last few hours of data.
